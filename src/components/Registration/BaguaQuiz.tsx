  ...
  const handleShareCard = async () => {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare) {
        ...
        const file = new File([blob], `新港八卦謎蹤_天命卡片_${result}.png`, {
          type: 'image/png'
        });
    ...
    <>
          ...
          >
            ...
            >
              ...
              <div
                style={{
                  ...
                  borderTop: '30px solid var(--accent-orange)',
                  zIndex: 10
                }}
              ></div>
                ...
                >
                  ...
                  {BAGUAS.map((b, i) => (
                    <div
                      ...
                      style={{
                        position: 'absolute',
                        transform: `rotate(${i * 45}deg) translateY(-115px)`,
                        fontSize: lang === 'en' ? '0.85rem' : '1.6rem',
                        fontWeight: 'bold',
                        color: '#c1a57b',
                        width: lang === 'en' ? '60px' : '45px',
                        textAlign: 'center',
                        textShadow: '0 2px 4px rgba(0,0,0,0.8)'
                      }}
                    >
                      {t?.trigramData?.[b]?.name || b}
                    </div>
                  ))}
            ...
            {!result ? (
              ...
              >
                ...
                >
                  {t?.quizResultTitle || '【 您的天命方位 】'}{' '}
                  {t?.trigramData?.[result]?.name || result}
                </h3>
                <p style={{ fontWeight: 'bold', marginBottom: '10px' }}>
                  {t?.quizRoleLabel || '性格：'}
                  {t?.trigramData?.[result]?.role || '---'}
                </p>
                <p
                  style={{
                    opacity: 0.9,
      ...
      >
        ...
        <div style={{ fontSize: '5rem', margin: '20px 0' }}>
          {result && (t.trigramData[result]?.name || result)}
        </div>
        <div
          style={{
            fontSize: '2rem',
        ...
        >
          <div
            style={{ fontSize: '0.9rem', opacity: 0.6, marginBottom: '5px' }}
          >
            {t.quizFateGuide}
          </div>
